...
class Sudoku():
    ...
    def set_candidates(self, nums, indices):
        ...
        erased = []
        for i, j in indices:
            old = self.candidates[i][j].intersection(nums) # beware triples where the whole triple is not in each box
    ...
    def box_line_reduction(self, inds_box):
        ...
        for i in range(i0, i1 +  1):
            ...
            line = self.get_candidates((i, 0), (i, j0 -1)) | self.get_candidates((i, j1 + 1), (i, self.n - 1)) 
        ...
        for j in range(j0, j1 + 1):
            ...
            line = self.get_candidates((0, j), (i0 - 1, j)) | self.get_candidates((i1 + 1, j), (self.n - 1, j)) 
        ...
        while not solved:
            ...
            if game.grid[i][j] == 0:
                ...
                for y in options:
                    ...
                    grid_final, solved = solve(game_next, ij=ij+1, depth=depth+1)
                    if solved:
                        break
